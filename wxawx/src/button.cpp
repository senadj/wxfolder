    ...
    wxWindow(parent, id, pos, size)
{
    ...
    if(upXPM) m_icons[0] = new wxIcon(upXPM);
    // if the over image is a NULL pointer, use a upper frame instead
    if(overXPM) m_icons[1] = new wxIcon(overXPM);
    // if the down image is a NULL pointer, use a lower frame instead
    if(downXPM) m_icons[2] = new wxIcon(downXPM);
    // without a disable image, the up image will be used
    if(disXPM) m_icons[3] = new wxIcon(disXPM);
    else {
	   if(upXPM) m_icons[3] = new wxIcon(upXPM);
    }
    // button font text
// #ifdef __WIN32__
...
// #endif    
    ...
    m_font = new wxFont( wxSystemSettings::GetFont( wxSYS_DEFAULT_GUI_FONT ) );

    if( m_font->GetPointSize() > 11 ) {
...
awxRadioButton::awxRadioButton(wxWindow * parent, wxWindowID id,
						 const wxPoint & pos,
						 const wxSize & size,
						 const char **upXPM,const char **overXPM,
						 const char **downXPM,const char **disXPM,
...
//			 if(m_box) m_box->ReleaseAll();
			 if(m_box) m_box->ReleaseAllExceptFor( GetId() );
			 m_snapin = true;
		  }
		  if(m_snapin) m_state = State_ButtonDown;
		  else m_state = State_ButtonOver;
