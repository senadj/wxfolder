...
void awxToolbar::AddSpacer( int width )
{
    m_sizer->SetDimension( 2, 0, width, m_size.y );
...
void awxToolbar::DrawBackground( wxDC& dc )
{
    int x, y;

    if( m_bgBitmap ) {

	   // tile drawing of smaller images
...
void awxToolbar::SetBackgroundImage( const char** bgXPM )
{
    m_bgBitmap = new wxBitmap( bgXPM );
}
